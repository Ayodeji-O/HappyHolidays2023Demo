...
void main() {
	...
	gl_FragColor = clamp(texture2D(uSampler, vTextureCoord) * totalLightContribution, 0.0, 1.0);
	
	//gl_FragColor = clamp((texelWithAmbient * pointLightColorWithContribution), 0.0, 1.0);
	
	
	//gl_FragColor = clamp(vec4(texelWithAmbient, 1.0) + pointLightColorWithContribution);
	
	
	// gl_FragColor = vec4(clamp((texture2D(uSampler, vTextureCoord) * (baseContribution + ambientContibution + pointLightContribution)).xyz, 0.0, 1.0), 1.0);
}